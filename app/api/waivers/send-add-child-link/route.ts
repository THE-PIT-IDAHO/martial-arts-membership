...
export async function POST(req: NextRequest) {
  try {
    ...
    const html = `
      <p>Hi ${memberName},</p>
      <p>${gymName} has sent you a link to add a child to your account.</p>
      <p>Please tap the link below to add the child's information and sign the waiver on their behalf:</p>
      <p><a href="${link}" style="display:inline-block;padding:12px 20px;background:#dc2626;color:#fff;text-decoration:none;border-radius:8px;font-weight:600">Add Child</a></p>
    ...
    const sent = await sendEmail({
      to: member.email,
      subject: `Add a child to your account at ${gymName}`,
      html,
    ...
    console.error("send-add-child-link error:", err);
